...
summary = "A checkers bot trained using the methods described in the original AlphaGO result."
...
In future research, the goal would be to parallelize this framework and enable it to train on cloud-deployed resource instances or, ideally, in-house computing resources. The memory and processing power needed to train the checkers framework fully  is not feasible on my CPU-powered laptop, so it's impossible to train the bot to any superhuman-level. Instead, we verified learning was taking place
